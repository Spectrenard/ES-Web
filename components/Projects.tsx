  ...
  return (
    <section className="py-12 md:py-20">
      <div className="container mx-auto px-4">
        <div
          ref={titleRef}
          className={`text-center mb-8 md:mb-16 opacity-0 ${
            ...
            return (
              <div
                ...
                className={`group relative overflow-hidden rounded-xl md:rounded-2xl 
                  bg-neutral-100 dark:bg-neutral-900 shadow-lg hover:shadow-2xl 
                  transition-all duration-500 ease-out opacity-0
                  ${projectInView ? "animate-fade-in-up" : ""}
                  ...
                  <div className="absolute bottom-0 p-4 md:p-8 w-full transform transition-all duration-500 ease-out">
                    <div className="flex flex-wrap gap-1.5 md:gap-2 mb-3 md:mb-4">
                      {project.tags.map((tag, tagIndex) => (
                        <span
                          key={tagIndex}
                          className="px-3 md:px-4 py-1 md:py-1.5 text-xs md:text-sm 
                            bg-white/10 backdrop-blur-md rounded-full text-white 
                            border border-white/10 shadow-sm transform transition-all 
                            duration-300 group-hover:bg-white/20 group-hover:scale-105"
                        >
                          {tag}
                        </span>
                      ))}
